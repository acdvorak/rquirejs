...
Compiler.prototype = {
    ...
    _addUniversal: function(runtime, config) {
        if (!config.universal) {
            return runtime;
        }

        var intro = _utils.readFileSyncRel('../runtime/universal-intro.tpl.js'),
            outro = _utils.readFileSyncRel('../runtime/universal-outro.tpl.js');

        intro = _utils.replace(intro, 'UNIVERSAL_MODULE_NAME', config.universal);
        outro = _utils.replace(outro, 'UNIVERSAL_MODULE_NAME', config.universal);
    ...
    _onScanComplete: function() {
        ...
        runtime = this._addUniversal(runtime, config);

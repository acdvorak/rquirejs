 ...
 * @param {Boolean}  [useMicroPaths=false]   If true, all require() paths in the compiled output file will be replaced with
 *                                           auto-incremented integers to improve minification.
 * @class
 * @constructor
 */
var DepTree = function(srcRootAbs, srcFilePathsRel, useMicroPaths) {
    srcFilePathsRel = srcFilePathsRel || [];
    useMicroPaths = (useMicroPaths === true);

    /** @type {Boolean} */
    this._useMicroPaths = useMicroPaths;

    /** @type {String} */
    this.srcRootAbs = srcRootAbs;

    /** @type {File[]} */
    this._initialSrcFiles = srcFilePathsRel.map(function(filePathRel) { return new File(srcRootAbs, filePathRel, useMicroPaths); });
...
DepTree.prototype = {
    ...
    _onData: function(fileInfo) {
        var file = new File(this.srcRootAbs, fileInfo.path, this._useMicroPaths);
    ...
    _buildTreeImpl: function(depFileMap, file) {
        ...
        var depFiles = file.directDependencies.map(function(pathRel) {
            var depFile = self._srcFileMap[pathRel];
            if (!depFile) {
                throw new Error('Required module "' + pathRel + '" not found in ' + file.pathAbs);
            }
            return depFile;
        });
